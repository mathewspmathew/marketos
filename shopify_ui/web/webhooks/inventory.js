...
// Forward inventory webhooks to MarketOS backend
const MARKETOS_WEBHOOK_URL = process.env.MARKETOS_WEBHOOK_URL || "https://your-marketos-domain.com/webhooks/shopify";

export async function inventoryWebhook(request) {
  const { topic, shop, body, webhookId } = request;
  
  console.log(`Inventory webhook received: ${topic} for shop: ${shop}`);
