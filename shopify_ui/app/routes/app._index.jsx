/**
 * app._index.jsx (Products) — default landing page at /app: Dynamic Pricing
 * product catalog (list, search/filter, per-product config panel — search
 * query, discovery settings, pricing tier, price bounds, rescrape frequency),
 * plus the manual product-sync trigger. calculateMinMaxPrices() is a
 * deliberate exception to "compute bounds in Python": it's a live
 * per-keystroke preview as the merchant edits the pricing tier, not the
 * authoritative clamp — decide.py still enforces the real lifetime-cap bound
 * server-side at pricing time.
 */
...
export const loader = async ({ request }) => {
  ...
  // Refresh/Retry button on this page owns recovery from ERROR).
  void fetch(
