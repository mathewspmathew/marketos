...
class PriceDecision(Base):
    ...
    shopifyResponse  = Column("shopifyResponse",  JSONB, nullable=True)
    changePct        = Column("changePct",        Float, nullable=True)
    refPrice         = Column("refPrice",         Numeric(10, 2), nullable=True)
    formulaTarget    = Column("formulaTarget",    Numeric(10, 2), nullable=True)
    competitorsUsed  = Column("competitorsUsed",  Integer, nullable=False, default=0)
    topMatchesJson   = Column("topMatchesJson",   JSONB, nullable=True)
    tierAtDecision   = Column("tierAtDecision",   _pricing_tier, nullable=True)
    skipReason       = Column("skipReason",       String, nullable=True)
    autoApplied      = Column("autoApplied",      Boolean, nullable=False, default=False)
    applyError       = Column("applyError",       String, nullable=True)
    oosObservations  = Column("oosObservations",  Integer, nullable=False, default=0)
    currencyDrops    = Column("currencyDrops",    Integer, nullable=False, default=0)
...
class ProductMatch(Base):
    ...
    textScore        = Column("textScore",        Numeric(5, 2), nullable=True)
    imageScore       = Column("imageScore",       Numeric(5, 2), nullable=True)
    confidence       = Column("confidence",       Numeric(4, 3), nullable=True)
    confidenceTier   = Column("confidenceTier",   _match_confidence_tier, nullable=True)
    dismissedAt      = Column("dismissedAt",      DateTime(timezone=True), nullable=True)
    currencyMismatch = Column("currencyMismatch", Boolean, nullable=False, default=False)
